...
## 占い
    ...
    execute if score hour temporary matches 1 unless entity @s[nbt={SelectedItem:{id:"minecraft:golden_horse_armor"}}] if score @s fortune_teller_heart matches 1.. unless score @s fortune_teller_heart_used matches 1 as @a[scores={PlayerID=5}] run function core:sign/system/fortune_teller_heart

## 騎士の加護
    execute if score hour temporary matches 1 if entity @s[nbt={SelectedItem:{id:"minecraft:golden_horse_armor"}}] unless entity @s[scores={PlayerID=5}] as @a[scores={PlayerID=5}] run function core:sign/system/the_blessing_of_knighthood
